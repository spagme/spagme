export interface ApiInterface {
  longNullable: (input: number | null, method?: string) => Promise<number | null>,
}

export abstract class ApiBase implements ApiInterface {
  abstract longNullable: (input: number | null, method?: string) => Promise<number | null>;
}
...
export class Api implements ApiBase {
  ...
  longNullable: (input: number | null, method?: string)  => Promise<number | null> = (input: number | null, method?: string) : Promise<number | null> => {
    const data: any = {};
    data.input = JSON.stringify(input);
    if(method === 'get') return this.get(`${this.url}/longNullable`, data);
    return this.post(`${this.url}/longNullable`, data);
  }
}
