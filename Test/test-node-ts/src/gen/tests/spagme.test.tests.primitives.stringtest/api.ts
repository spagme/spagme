export interface ApiInterface {
  string: (input: string | null, method?: string) => Promise<string | null>,
}

export abstract class ApiBase implements ApiInterface {
  abstract string: (input: string | null, method?: string) => Promise<string | null>;
}
...
export class Api implements ApiBase {
  ...
  string: (input: string | null, method?: string)  => Promise<string | null> = (input: string | null, method?: string) : Promise<string | null> => {
    const data: any = {};
    data.input = JSON.stringify(input);
    if(method === 'get') return this.get(`${this.url}/string`, data);
    return this.post(`${this.url}/string`, data);
  }
}
