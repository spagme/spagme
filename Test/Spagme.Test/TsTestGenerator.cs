...
{
    public class TsTestGenerator
    {
        private readonly string _dir;
        private readonly string _testdir;

        public TsTestGenerator(string dir)
        {
            _dir = dir;
            _testdir = dir + "/tests";
        }
        ...
        public void GenerateApi()
        {
            ...
            foreach (var type in assembly.DefinedTypes.Where(o => o.ImplementedInterfaces.Contains(typeof(ITest))))
            {
                try
                {
                    ...
                    var str = SpagmeGen.Ts(type, new TsGenOptions()
                    {
                        ApiClassName = "Api",
                        DisableLint = false,
                    });
                    File.WriteAllText(dirname + "/api.ts", str);
        ...
        public void GenerateTests()
        {
            ...
            foreach (var type in assembly.DefinedTypes.Where(o => o.ImplementedInterfaces.Contains(typeof(ITest))))
            {
                ...
                code.AppendLine("import * as api from './api';");
                code.AppendLine("import Test from '../../../Test';");
                code.AppendLine("// eslint-disable-next-line");
                code.AppendLine("import { diff } from 'deep-object-diff';");
                code.AppendLine();
                code.AppendLine("export const tests: Array<Test> = [];");
                code.AppendLine();
                ...
                File.WriteAllText(dirname + "/tests.ts", code.ToString());
            }

            var tmp = new StringBuilder();
            tmp.AppendLine("import Test from '../../Test';");

            int count = 1;
            foreach (var type in assembly.DefinedTypes.Where(o => o.ImplementedInterfaces.Contains(typeof(ITest))))
            {
                var fullname = type.FullName.ToLower();
                tmp.AppendLine($"import {{tests as t{count}}} from './{fullname}/tests';");

                count++;
            }

            tmp.AppendLine("const tests: Array<Test> = [");
            ...
            File.WriteAllText(_testdir + "/tests.ts", tmp.ToString());
        }
        ...
        public string GenerateTest(MethodInfo methodInfo, string fullname, string testname, Func<Type,int,string> gen, string method)
        {
            ...
            code.AppendLine(Intedent(2) + "execute: async (me: Test, url: string): Promise<void> => {");
            ...
            {
                code.AppendLine(Intedent(4) + $"const input: {GetInputType(input)}{Null(input)} = {gen.Invoke(input, 0)};");
                code.AppendLine(Intedent(4) + "me.input = input; ");
                code.AppendLine(Intedent(4) + $"const output = await new api.Api(url + '/{fullname}').{ReflectionUtil.ToCamelCase(methodInfo.Name)}(input, '{method}');  ");
            ...
            else
            {
                ...
                code.AppendLine(Intedent(4) + "me.output = {...output};");
                code.AppendLine(Intedent(4) + "me.status = 'ERROR';");
                code.AppendLine(Intedent(4) + "if(Object.keys(diff(input, output as object)).length === 0) me.status = 'SUCCESS';");
            }
